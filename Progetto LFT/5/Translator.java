...
public class Translator {
    ...
    void move() {// stampa a video i token
    ...
    void match(int t) {
        if (look.tag == t) {
            if (look.tag != Tag.EOF) move();//a stampare a move tutti i tag
    ...
    public void statp(int lnext) {// controllo tutti i casi
        int lnext_true;
        int lnext_false;
        int read_id_addr;
        switch (look.tag) {
            ...
            case '=':
                ...
                if (read_id_addr == -1) {
                    read_id_addr = count;
                    st.insert(((Word) look).lexeme, count++);//insert in SymbleTable
                }
                match(Tag.ID);
                expr();
                code.emit(OpCode.istore, read_id_addr);//crea  l'ijvm giusto
                break;
            default:
